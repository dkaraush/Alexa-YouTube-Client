	...
	},
	{
		...
		_handle: async function (RI, handlerInput, user, slots, res, hasDisplay, hasVideoApp) {
			...
			*/
			...
			return res.addAudioPlayerPlayDirective("REPLACE_ALL", redirectVideo(youtubelink), data.link.id, 0, null, metadata(data.pitems[data.index]))
		}
	},
	{
...
async function runVideo(RI, requestname, data, cantalk, behavior, type, youtube, user, res, speech) {
	if (data.index == 0 && type) {
		return new Promise((resolve, reject) => {
			var playlist = data.pitems.slice(0, 2);
